...
BEGIN_MESSAGE_MAP(CEntornVGIView, CView)
	...
	ON_UPDATE_COMMAND_UI(ID_OBJECTE_ARC, &CEntornVGIView::OnUpdateObjecteArc)
		...
		ON_COMMAND(ID_OBJECTE_CAMIO32876, &CEntornVGIView::OnObjecteCamio32876)
		ON_UPDATE_COMMAND_UI(ID_OBJECTE_CAMIO32876, &CEntornVGIView::OnUpdateObjecteCamio32876)
		ON_COMMAND(ID_OBJECTE_AVIO, &CEntornVGIView::OnObjecteAvio)
		ON_UPDATE_COMMAND_UI(ID_OBJECTE_AVIO, &CEntornVGIView::OnUpdateObjecteAvio)
		...
		ON_COMMAND(ID_BXmenys, &CEntornVGIView::OnBxmenys)
		ON_COMMAND(ID_BXmes, &CEntornVGIView::OnBxmes)
		ON_COMMAND(ID_BXMmenys, &CEntornVGIView::OnBxmmenys)
		ON_COMMAND(ID_BXMmes, &CEntornVGIView::OnBxmmes)
		ON_COMMAND(ID_BYmenys, &CEntornVGIView::OnBymenys)
		ON_COMMAND(ID_BYmes, &CEntornVGIView::OnBymes)
		ON_COMMAND(ID_BZmenys, &CEntornVGIView::OnBzmenys)
		ON_COMMAND(ID_BZmes, &CEntornVGIView::OnBzmes)
		ON_COMMAND(ID_BZMmenys, &CEntornVGIView::OnBzmmenys)
		ON_COMMAND(ID_BZMmes, &CEntornVGIView::OnBzmmes)
		END_MESSAGE_MAP()
...
// Entorn VGI: Variables de control per Menú Vista: canvi PV interactiu, Zoom, dibuixar eixos i grid 
	mobil = true;	zzoom = true;	satelit = false;	pan = false;	navega = false;		eixos = true;	eixos_programID = 0;  eixos_Id = 0;
	sw_grid = false;
...
// Entorn VGI: Altres variables
	mida = 1.0;			nom = "";		buffer = "";    D = 1.0;    S = 1.0;    d = (7.0 / 4.0) * sqrt(3);

//Variables pràctica 3 robot ------ angles_R conté 0:alphaz, 1:alphax, 2:betaz, 3:betax, 4:betay, 5:gammax (opcional) 
	
	//inicialment l'increment està a 4 
	increment_angle = 4.0f;

	//inicialitzem els angles de rotació i els vectors d'animació a 0.0 graus 
	for (int i = 0; i < 6; i++) {
		angles_R[i] = 0.0f;
		pos_RI[i] = 0.0f;
		pos_RF[i] = 0.0f;
	...
	//inicialitzem el mode de treball en mode A
	mode_treball = 'A';

	//inicialitza el controlador d'insert en 1
	control_insert = 1;

	//controla l'orientació de l'animació
	control_orientacio = 1;
...
// PLANTA (Inferior Esquerra)
		...
		configura_Escena();
		GTMatrix = glm::scale(GTMatrix, glm::vec3(S, S, S));
		dibuixa_Escena();
...
//	Dibuix geometria de l'escena amb comandes GL.
	dibuixa_EscenaGL(shader_programID, eixos, eixos_Id, grid, hgrid, objecte, col_obj, true, sw_material, textura, texturesID, textura_map,
		npts_T, PC_t, pas_CS, sw_Punts_Control, ViewMatrix, GTMatrix, angles_R);
...
// TODO: Agregue aquí su código de controlador de mensajes o llame al valor predeterminado
	...
	if (objecte != ROBOT) {
		...
		//else if (nChar == 'a') angleRot += 10.0f;
		...
		//Si estem en A fem rotació negativa del colze a Z, si estem en B fem rotació positiva del canell respecte Z
		case VK_RIGHT:

			if (mode_treball == 'A') {
				if (angles_R[0] - increment_angle >= 0)	angles_R[0] -= increment_angle;
				else  angles_R[0] = 360 - (increment_angle - (360 - angles_R[0]));
			}
			else {
				if (angles_R[2] - increment_angle >= 0)	angles_R[2] -= increment_angle;
				else  angles_R[2] = 360 - (increment_angle - (360 - angles_R[2]));
			}
			break;

		//Si estem en mode A fem rotació positiva del colze a Z, si estem en B fem rotació positiva del canell respecte Z
		case VK_LEFT:

			if (mode_treball == 'A') {
				if (angles_R[0] + increment_angle <= 360) angles_R[0] += increment_angle;
				else angles_R[0] = (increment_angle - (360 - angles_R[0]));
			}
			else {
				if (angles_R[2] + increment_angle <= 360) angles_R[2] += increment_angle;
				else angles_R[2] = (increment_angle - (360 - angles_R[2]));
			}
			break;

		//Si estem en mode A fem rotació negativa del colze a l'eix X, si estem en B fem rotació positiva del canell respecte X
		case VK_DOWN:

			if (mode_treball == 'A') {
				if (angles_R[1] - increment_angle >= 0)	angles_R[1] -= increment_angle;
				else  angles_R[1] = 360 - (increment_angle - (360 - angles_R[1]));
			}
			else {
				if (angles_R[3] + increment_angle <= 360) angles_R[3] += increment_angle;
				else angles_R[3] = (increment_angle - (360 - angles_R[3]));
			}
			break;
		
		//Si estem em mode A fem rotació positiva a l'eix X del colze, si estem en mode B fem rotació negativa a l'eix X del canell.
		case VK_UP:
			
			if (mode_treball == 'A') {
				if (angles_R[1] + increment_angle <= 360) angles_R[1] += increment_angle;
				else angles_R[1] = (increment_angle - (360 - angles_R[1]));
			}
			else {
				if (angles_R[3] - increment_angle >= 0)	angles_R[3] -= increment_angle;
				else  angles_R[3] = 360 - (increment_angle - (360 - angles_R[3]));
			}
			break;
		
		//En mode de treball A no fa res (opcional), en mode de treball B rotació canell respecte Y (positiva)
		case VK_HOME:

			if (mode_treball == 'B') {
				if (angles_R[4] + increment_angle <= 360) angles_R[4] += increment_angle;
				else angles_R[4] = (increment_angle - (360 - angles_R[4]));
			}
			break;

		//En mode treball A no fa res (opcional), en mode treball B rotació canell respecte Y (negativa)
		case VK_END:

			if (mode_treball == 'B') {
				if (angles_R[4] - increment_angle >= 0)	angles_R[4] -= increment_angle;
				else  angles_R[4] = 360 - (increment_angle - (360 - angles_R[4]));
			}
			break;

		//Canvia el mode de treball entre A i B
		case VK_SPACE:
			if (mode_treball == 'A') mode_treball = 'B';
			else mode_treball = 'A';
			break;
		
		//Multiplica per dos l'increment tenint en compte 256 com a límit
		case VK_NEXT:
			if (increment_angle != 256.0f) increment_angle *= 2.0f;
			break;

		//Divideix per dos l'increment tenenint en compte 1 com a límit
		case VK_PRIOR:
			if (increment_angle != 1.0f) increment_angle /= 2.0f;
			break;

		//Primera vegada que fem click guarda posició inicial de l'animació (pos_RI) i la segona guarda l'estat final (pos_RF) si son diferents. A partir d'aquestes fem reset i tornem a gravar la primera, segona etc
		case VK_INSERT:
			
			if (control_insert == 1) {
				
				//Guarda en pos_RI la posició actual
				for (int i = 0; i < 6; i++) {
					pos_RI[i] = angles_R[i];
				}
				control_insert++;
				...
				//Fem check de si son iguals per a descartar la segona posició o bé agafarla
				bool son_iguals = true;
				
				for (int i = 0; i < 6; i++) {
					if (angles_R[i] != pos_RI[i]) {
						son_iguals = false;
					}
				}
		...
		//Si animació no activa, l'activem i si està activa la pausem i reiniciem el valor per a tornar a començar en la posició inicial
		case VK_F5:
			if (anima == false) {
				t = 0;
				anima = true;
		...
		//Si animació no activa, l'activem i si està activa la pausem però no reiniciem el valor perque al tornar a fer clic continui desde la posició de pausa
		case VK_PAUSE:
			if (anima == false) {
				anima = true;
				SetTimer(WM_TIMER, 4, NULL);
...
// TODO: Agregue aquí su código de controlador de mensajes o llame al valor predeterminado
	if (anima)	{
		...
		//posar un if a cadascu i aplicar formula segons
		angles_R[0] = pos_RI[0] + ((t * (pos_RF[0] - pos_RI[0])) / N_FRAMES);
		angles_R[1] = pos_RI[1] + ((t * (pos_RF[1] - pos_RI[1])) / N_FRAMES);
		angles_R[2] = pos_RI[2] + ((t * (pos_RF[2] - pos_RI[2])) / N_FRAMES);
		angles_R[3] = pos_RI[3] + ((t * (pos_RF[3] - pos_RI[3])) / N_FRAMES);
		angles_R[4] = pos_RI[4] + ((t * (pos_RF[4] - pos_RI[4])) / N_FRAMES);
...
//	Canviar l'escala per a centrar la vista (Ortogràfica)
	//necessitem una esfera amb radi diagonal_cub/2
	double diagonal = sqrt(3) * 5;
	double radi = diagonal / 2;
	OPV.R = (radi)/(sin(30*PI/180));
	
//sabem que el cub inicialment és d'1x1x1 pero se li fa un escalatge per a que sigui 5x5x5, per tant D és la diagonal d'un cub 5x5x5 fem 7.5x7.5x7.5 per a que es vegi amb marges
	D = sqrt(3) * 7.5;
	S = d / D;
...
//  Modificar R per centrar la Vista a la mida de l'objecte (Perspectiva)
//	Canviar l'escala per a centrar la vista (Ortogràfica)
	//l'esfera directament "cap" dins una esfera del seu propi tamany, és a dir, radi 5
	OPV.R = (5 / sin(30*PI/180));

//En el cas de l'esfera també la reescalen però ja que al seleccionar 1 quan cridem glSolidSphere ens referim a 1 de radi implica que el radi al ser escalat
//és de 5, per tant, l'esfera té diàmetre de 10, és a dir, necessitem un cub de 10x10x10 per a fer l'escalatge
	D = sqrt(3) * 10;
	S = d / D;
...
//  Modificar R per centrar la Vista a la mida de l'objecte (Perspectiva)
//	Canviar l'escala per a centrar la vista (Ortogràfica)
	OPV.R = (6 / sin(30 * PI / 180));

//La tetera és semblant a l'esfera, necessitem un cub de 10x10x10 si volem representarla sencera
	D = sqrt(3) * 10;
	S = d / D;
...
void CEntornVGIView::OnObjecteCamio32876()
{
	// TODO: Add your command handler code here
	objecte = CAMIO;

	double diagonal = sqrt(3) * 10;
	double radi = diagonal / 2;
	OPV.R = (radi) / (sin(30 * PI / 180));
...
void CEntornVGIView::OnUpdateObjecteCamio32876(CCmdUI* pCmdUI)
{
	// TODO: Add your command update UI handler code here
...
void CEntornVGIView::OnObjecteAvio()
{
	// Canvia l'objecte a avió i el dibuixa
	objecte = AVIO;
	double diagonal = sqrt(3) * 60;
	double radi = diagonal / 2;
	OPV.R = (radi) / (sin(30 * PI / 180));
...
void CEntornVGIView::OnUpdateObjecteAvio(CCmdUI* pCmdUI)
{
	// Fa update sobre el check
	if (objecte == AVIO) pCmdUI->SetCheck(1);
	else pCmdUI->SetCheck(0);
}
...
{
	//Posem la constant relativa a bricks a la variable t_textura
	t_textura = BRICKS;

	//Associem textura bricks amb la imatge brick.jpg directament
	char* path = "..\\EntornVGI\\textures\\brick.jpg";
	
	// Entorn VGI: Activació el contexte OpenGL
	wglMakeCurrent(m_pDC->GetSafeHdc(), m_hRC);

	texturesID[0] = loadIMA_SOIL(path);
...
void CEntornVGIView::OnUpdateTexturaBricks(CCmdUI* pCmdUI)
{
	...
	if (t_textura == BRICKS) pCmdUI->SetCheck(1);
	else pCmdUI->SetCheck(0);
}
...
{
	//Posem la constant relativa a sac a la variable t_textura
	t_textura = SAC;

	//Associem textura sac amb la imatge sac.jpg directament
	char* path = "..\\EntornVGI\\textures\\sac.jpg";
...
{
	if (t_textura == SAC) pCmdUI->SetCheck(1);
	else pCmdUI->SetCheck(0);
}
...
void CEntornVGIView::OnTexturaMapamundi()
{
	...
	//Associem textura bricks amb la imatge brick.jpg directament
	char* path = "..\\EntornVGI\\textures\\world-political-map-with-countries.jpg";
...
{
	if (t_textura == MAPAMUNDI) pCmdUI->SetCheck(1);
	else pCmdUI->SetCheck(0);
}
...
//X_majuscula -
void CEntornVGIView::OnBxmenys()
{
	// Resta increment_angle a l'angle alfa x tenint en compte que no es generi un angle negatiu
	if (angles_R[1] - increment_angle >= 0)	angles_R[1] -= increment_angle;
	else  angles_R[1] = 360 - (increment_angle - (360 - angles_R[1]));
	InvalidateRect(NULL, false);
}
...
//X_minuscula -
void CEntornVGIView::OnBxmmenys()
{
	// TODO: Add your command handler code here
	if (angles_R[3] - increment_angle >= 0)	angles_R[3] -= increment_angle;
	else  angles_R[3] = 360 - (increment_angle - (360 - angles_R[3]));
...
//Z_minuscula -
void CEntornVGIView::OnBzmmenys()
{
	// TODO: Add your command handler code here
	if (angles_R[2] - increment_angle >= 0)	angles_R[2] -= increment_angle;
	else  angles_R[2] = 360 - (increment_angle - (360 - angles_R[2]));

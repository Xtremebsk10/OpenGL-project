...
//			        QUE ES CARREGUEN A LA MATRIU DE PROJECCIÓ GL_PROJECTION
	...
	//MatriuProjeccio = glm::ortho(-10.0, 10.0, -10.0, 10.0, p_near, p_far); //Pràctica 1
	 //Pràctica 2: Volum de visualització d'un cub de 2x2 centrat a origen

	if(w <= h) MatriuProjeccio = glm::ortho(-1.0, 1.0, -1.0 * h/w, 1.0 * h/w, p_near, p_far);
	else MatriuProjeccio = glm::ortho(-1.0 * w/h, 1.0 * w/h, -1.0, 1.0, p_near, p_far);
	//MatriuProjeccio = glm::ortho(-1.0, 1.0, -1.0, 1.0, p_near, p_far);
...
//								tipus de projecció definit a la variable prj.
	...
	case 0: //Planta
		MatriuVista = glm::lookAt(
			glm::vec3(0, 10, 0), // Camera is here
	...
	case 1: //Alçat
		MatriuVista = glm::lookAt(
			glm::vec3(10, 0, 0), // Camera is here
			glm::vec3(0, 0, 0), // and looks here
...
// Transformacions geometriques sobre objecte (Traslacio, Rotacions i Escalatge)
	...
	MatriuTG = glm::scale(MatriuTG, vec3(tg.VScal.x, tg.VScal.y, tg.VScal.z));	// Escalatge fixada amb la INSERT dins l'opcio Transformacions
/*
	if (TR) glTranslatef(tg.VTras.x, tg.VTras.y, tg.VTras.z);
	glTranslatef(tgF.VTras.x, tgF.VTras.y, tgF.VTras.z);	// Traslacio fixada amb la INSERT dins l'opcio Transformacions
	if (TR) glRotatef(tg.VRota.x, 1, 0, 0);
	glRotatef(tgF.VRota.x, 1, 0, 0);					// Rotacio X fixada amb la INSERT dins l'opcio Transformacions
	if (TR) glRotatef(tg.VRota.y, 0, 1, 0);
	glRotatef(tgF.VRota.y, 0, 1, 0);					// Rotacio Y fixada amb la INSERT dins l'opcio Transformacions
	if (TR) glRotatef(tg.VRota.z, 0, 0, 1);
	glRotatef(tgF.VRota.z, 0, 0, 1);					// Rotacio Z fixada amb la INSERT dins l'opcio Transformacions
	if (TR) glScalef(tg.VScal.x, tg.VScal.y, tg.VScal.z);
	glScalef(tgF.VScal.x, tgF.VScal.y, tgF.VScal.z);	// Escalatge fixada amb la INSERT dins l'opcio Transformacions
*/
	return MatriuTG;
}

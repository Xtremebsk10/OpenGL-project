...
// Camio
	...
	case AVIO:
		...
		//funcio que dibuixa l'avio
		avio(sh_programID, MatriuVista, MatriuTG, sw_mat);
		break;

	case ROBOT:
		robot(sh_programID, MatriuVista, MatriuTG, sw_mat, angles_R);
		break;
...
// Corba Bezier
	case C_BEZIER:
		...
		{	// Definició propietats de reflexió (emissió, ambient, difusa, especular) del material pel color vermell.
			SeleccionaColor(sh_programID, color_vermell, sw_material);
			npunts = gluSphere_VAO(5.0, 20, 20);
			...
			{	//glPushMatrix();
				  ...
				  drawgluSphere(npunts);	//gluSphere(5.0, 20, 20);
				//glPopMatrix();
			}
...
{
	//Objecte de color i definició de les matrius
	CColor col_object;
	...
	SeleccionaColor(sh_programID, col_object, sw_mat);
	ModelMatrix = glm::translate(MatriuTG, vec3(0.0, 0.0, 1.3));
	...
	//Cabina del conductor (eix y 2.5+0.5; eix z 0.5)
	col_object.r = 0.0;	col_object.g = 0.7;		col_object.b = 0.7;	 col_object.a = 1.0;
	SeleccionaColor(sh_programID, col_object, sw_mat);
	ModelMatrix = glm::translate(MatriuTG, vec3(0.0, 3.0, 0.8));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
	NormalMatrix = transpose(inverse(MatriuVista * ModelMatrix));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "normalMatrix"), 1, GL_FALSE, &NormalMatrix[0][0]);
	glutSolidCube(1.0);
	...
	SeleccionaColor(sh_programID, col_object, sw_mat);
	ModelMatrix = glm::translate(MatriuTG, vec3(0.7, 2.0, 0.3));
...
void robot(GLuint sh_programID, glm::mat4 MatriuVista, glm::mat4 MatriuTG, bool sw_mat[4], float angles_R[6]) {
	
	//Objecte de color i definició de les matrius
	CColor col_object;
	glm::mat4 NormalMatrix(1.0), ModelMatrix(1.0);

	//MatriuTG = glm::translate(MatriuTG, vec3(0.0, 0.0, 0.0));

	//Base 
	col_object.r = 0.0;	col_object.g = 0.0;		col_object.b = 1.0;	 col_object.a = 1.0;
	SeleccionaColor(sh_programID, col_object, sw_mat);
	ModelMatrix = glm::scale(MatriuTG, vec3(10.0f, 10.0f, 5.0f));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
	NormalMatrix = transpose(inverse(MatriuVista * ModelMatrix));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "normalMatrix"), 1, GL_FALSE, &NormalMatrix[0][0]);
	glutSolidCube(1.0);
	...
	//Cama
	col_object.r = 1.0;	col_object.g = 0.0;		col_object.b = 1.0;	 col_object.a = 1.0;
	SeleccionaColor(sh_programID, col_object, sw_mat);
	ModelMatrix = glm::translate(MatriuTG, vec3(0.0, 0.0, 2.5));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
	NormalMatrix = transpose(inverse(MatriuVista * ModelMatrix));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "normalMatrix"), 1, GL_FALSE, &NormalMatrix[0][0]);
	glutSolidCone(2.0, 12.5, 20, 20);

	//Per a rotar tot el braç-colze, canell i pinça (Z)
	MatriuTG = glm::rotate(MatriuTG, radians(angles_R[0]), vec3(0.0, 0.0, 1.0f));

	//Per rotar tot el braç-colze, canell i pinça (X)
	MatriuTG = glm::translate(MatriuTG, vec3(0.0, 0.0, 15.0));
	MatriuTG = glm::rotate(MatriuTG, radians(angles_R[1]), vec3(1.0, 0.0, 0.0f));
	MatriuTG = glm::translate(MatriuTG, vec3(0.0, 0.0, -15.0));

	//Colze
	col_object.r = 1.0;	col_object.g = 0.0;		col_object.b = 0.0;	 col_object.a = 1.0;
	SeleccionaColor(sh_programID, col_object, sw_mat);
	ModelMatrix = glm::translate(MatriuTG, vec3(0.0, 0.0, 15.0));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
	NormalMatrix = transpose(inverse(MatriuVista * ModelMatrix));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "normalMatrix"), 1, GL_FALSE, &NormalMatrix[0][0]);
	...
	SeleccionaColor(sh_programID, col_object, sw_mat);
	ModelMatrix = glm::translate(MatriuTG, vec3(0.0, 0.0, 15.0));
	ModelMatrix = glm::rotate(ModelMatrix, radians(90.f), vec3(1.0, 0.0, 0.0f));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
	NormalMatrix = transpose(inverse(MatriuVista * ModelMatrix));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "normalMatrix"), 1, GL_FALSE, &NormalMatrix[0][0]);
	gluCylinder(0.5f, 0.5f, 15.0f, 20, 1);

	//Per rotar tot el bloc canell-pinça (Z) 2
	MatriuTG = glm::translate(MatriuTG, vec3(0.0, -15.0, 0.0));
	MatriuTG = glm::rotate(MatriuTG, radians(angles_R[2]), vec3(0.0, 0.0, 1.0f));
	MatriuTG = glm::translate(MatriuTG, vec3(0.0, 15.0, 0.0));

	//Per rotar tot el bloc canell-pinça (X) 3
	MatriuTG = glm::translate(MatriuTG, vec3(0.0, -15.0, 15.0));
	MatriuTG = glm::rotate(MatriuTG, radians(angles_R[3]), vec3(1.0, 0.0, 0.0f));
	MatriuTG = glm::translate(MatriuTG, vec3(0.0, 15.0, -15.0));

	//Per a rotar tot el bloc canell-pinça (Y) 4
	MatriuTG = glm::translate(MatriuTG, vec3(0.0, 0.0, 15));
	MatriuTG = glm::rotate(MatriuTG, radians(angles_R[4]), vec3(0.0, 1.0, 0.0f));
	MatriuTG = glm::translate(MatriuTG, vec3(0.0,0.0,-15));

	//Canell
	col_object.r = 1.0;	col_object.g = 0.0;		col_object.b = 0.0;	 col_object.a = 1.0;
	SeleccionaColor(sh_programID, col_object, sw_mat);
	ModelMatrix = glm::translate(MatriuTG, vec3(0.0, -15.0, 15.0));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
	NormalMatrix = transpose(inverse(MatriuVista * ModelMatrix));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "normalMatrix"), 1, GL_FALSE, &NormalMatrix[0][0]);
	glutSolidSphere(1.0, 20, 20);

	//Dit superior (part llarga)
	col_object.r = 0.0;	col_object.g = 0.5;		col_object.b = 0.0;	 col_object.a = 1.0;
	SeleccionaColor(sh_programID, col_object, sw_mat);
	ModelMatrix = glm::translate(MatriuTG, vec3(0.0, -15.0-(sqrt(2)*(1.75 / 2.0)), (sqrt(2)*(1.75/2.0))+15.0));
	ModelMatrix = glm::rotate(ModelMatrix, radians(45.0f), vec3(1.0, 0.0, 0.0f));
	ModelMatrix = glm::scale(ModelMatrix, vec3(1.0f, 0.5f, 4.0f));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
	NormalMatrix = transpose(inverse(MatriuVista * ModelMatrix));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "normalMatrix"), 1, GL_FALSE, &NormalMatrix[0][0]);
	glutSolidCube(1.0);

	//Dit superior part curta
	col_object.r = 0.0;	col_object.g = 0.5;		col_object.b = 0.0;	 col_object.a = 1.0;
	SeleccionaColor(sh_programID, col_object, sw_mat);
	ModelMatrix = glm::translate(MatriuTG, vec3(0.0, -15.0 - (sqrt(2) * (1.75 / 1.0)) -(sqrt(2)*(0.75/2.0)), (sqrt(2) * (1.75 / 1.0)) + 15.0 - (sqrt(2) * (0.75 / 2.0))));
	ModelMatrix = glm::rotate(ModelMatrix, radians(135.0f), vec3(1.0, 0.0, 0.0f));
	ModelMatrix = glm::scale(ModelMatrix, vec3(1.0f, 0.5f, 2.0f));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
	NormalMatrix = transpose(inverse(MatriuVista * ModelMatrix));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "normalMatrix"), 1, GL_FALSE, &NormalMatrix[0][0]);
	glutSolidCube(1.0);
	
	//Dit inferior (part llarga)
	col_object.r = 0.0;	col_object.g = 0.0;		col_object.b = 0.5;	 col_object.a = 1.0;
	SeleccionaColor(sh_programID, col_object, sw_mat);
	ModelMatrix = glm::translate(MatriuTG, vec3(0.0, -15.0-(sqrt(2)*(1.75 / 2.0)), 15.0-(sqrt(2) * (1.75 / 2.0))));
	ModelMatrix = glm::rotate(ModelMatrix, radians(135.0f), vec3(1.0, 0.0, 0.0f));
	ModelMatrix = glm::scale(ModelMatrix, vec3(1.0f, 0.5f, 4.0f));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
	NormalMatrix = transpose(inverse(MatriuVista * ModelMatrix));
...
void avio(GLuint sh_programID, glm::mat4 MatriuVista, glm::mat4 MatriuTG, bool sw_mat[4]) {
	...
	SeleccionaColor(sh_programID, col_object, sw_mat);
	ModelMatrix = glm::translate(MatriuTG, vec3(0.0, 27.5, 5.0));
	ModelMatrix = glm::scale(ModelMatrix, vec3(2.0f, 1.8f, 2.0f));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
	NormalMatrix = transpose(inverse(MatriuVista * ModelMatrix));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "normalMatrix"), 1, GL_FALSE, &NormalMatrix[0][0]);
	glutSolidSphere(1.0, 20, 20);
	...
	col_object.r = 1.0;	col_object.g = 1.0;		col_object.b = 1.0;	 col_object.a = 1.0;
	SeleccionaColor(sh_programID, col_object, sw_mat);
	ModelMatrix = glm::translate(MatriuTG, vec3(0.0, 17.5, 5.0));
	ModelMatrix = glm::rotate(ModelMatrix, radians(270.f), vec3(1.0, 0.0, 0.0f));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
	NormalMatrix = transpose(inverse(MatriuVista * ModelMatrix));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "normalMatrix"), 1, GL_FALSE, &NormalMatrix[0][0]);
	gluCylinder(5.0f, 2.0f, 10.5f, 20, 1);
	...
	NormalMatrix = transpose(inverse(MatriuVista * ModelMatrix));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "normalMatrix"), 1, GL_FALSE, &NormalMatrix[0][0]);
	gluCylinder(5.0f, 5.0f, 35.0f, 20, 1);
	...
	NormalMatrix = transpose(inverse(MatriuVista * ModelMatrix));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "normalMatrix"), 1, GL_FALSE, &NormalMatrix[0][0]);
	gluCylinder(5.0f, 1.0f, 12.5f, 20, 1);

	//Cul avió esfera
	col_object.r = 1.0;	col_object.g = 1.0;		col_object.b = 1.0;	 col_object.a = 1.0;
	SeleccionaColor(sh_programID, col_object, sw_mat);
	ModelMatrix = glm::translate(MatriuTG, vec3(0.0, -30.0, 5.0));
	//ModelMatrix = glm::scale(ModelMatrix, vec3(2.0f, 1.8f, 2.0f));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
	NormalMatrix = transpose(inverse(MatriuVista * ModelMatrix));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "normalMatrix"), 1, GL_FALSE, &NormalMatrix[0][0]);
	...
	//Ala 1 ESTRUCTURA PRINCIPAL (CUB)
	col_object.r = 0.972;	col_object.g = 0.964;		col_object.b = 0.964;	 col_object.a = 1.0;
	SeleccionaColor(sh_programID, col_object, sw_mat);
	ModelMatrix = glm::translate(MatriuTG, vec3(15.0, 0, 5.0));
	ModelMatrix = glm::scale(ModelMatrix, vec3(20.0f, 6.0f, 0.6f));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
	NormalMatrix = transpose(inverse(MatriuVista * ModelMatrix));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "normalMatrix"), 1, GL_FALSE, &NormalMatrix[0][0]);
	glutSolidCube(1.0);

	//Ala 1 ESTRUCTURA PUNTA (esfera)
	col_object.r = 0.972;	col_object.g = 0.964;		col_object.b = 0.964;	 col_object.a = 1.0;
	SeleccionaColor(sh_programID, col_object, sw_mat);
	ModelMatrix = glm::translate(MatriuTG, vec3(25.0, 0.0, 5.0));
	ModelMatrix = glm::scale(ModelMatrix, vec3(3.0f, 3.0f, 0.35f));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
	NormalMatrix = transpose(inverse(MatriuVista * ModelMatrix));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "normalMatrix"), 1, GL_FALSE, &NormalMatrix[0][0]);
	glutSolidSphere(1.0, 20, 20);

	//Ala 2 ESTRUCTURA PRINCIPAL (CUB)
	col_object.r = 0.972;	col_object.g = 0.964;		col_object.b = 0.964;	 col_object.a = 1.0;
	SeleccionaColor(sh_programID, col_object, sw_mat);
	ModelMatrix = glm::translate(MatriuTG, vec3(-15.0, 0, 5.0));
	ModelMatrix = glm::scale(ModelMatrix, vec3(20.0f, 6.0f, 0.6f));
	...
	//Propulsor 1 part principal (cilindre)
	col_object.r = 0.811;	col_object.g = 0.811;		col_object.b = 0.811;	 col_object.a = 1.0;
	SeleccionaColor(sh_programID, col_object, sw_mat);
	ModelMatrix = glm::translate(MatriuTG, vec3(10.0, 0.0, 3.4));
	ModelMatrix = glm::rotate(ModelMatrix, radians(270.f), vec3(1.0, 0.0, 0.0f));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
	NormalMatrix = transpose(inverse(MatriuVista * ModelMatrix));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "normalMatrix"), 1, GL_FALSE, &NormalMatrix[0][0]);
	gluCylinder(1.5f, 1.5f, 6.0f, 20, 1);

	//Propulsor 1 detall davanter (torus)
	col_object.r = 0.49;	col_object.g = 0.49;		col_object.b = 0.49;	 col_object.a = 1.0;
	SeleccionaColor(sh_programID, col_object, sw_mat);
	ModelMatrix = glm::translate(MatriuTG, vec3(10.0, 6.0, 3.4));
	ModelMatrix = glm::rotate(ModelMatrix, radians(270.f), vec3(1.0f, 0.0f, 0.0f));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
	NormalMatrix = transpose(inverse(MatriuVista * ModelMatrix));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "normalMatrix"), 1, GL_FALSE, &NormalMatrix[0][0]);
	glutSolidTorus(0.2, 1.3, 32, 32);


	//Propulsor 2 part principal (cilindre)
	col_object.r = 0.811;	col_object.g = 0.811;		col_object.b = 0.811;	 col_object.a = 1.0;
	SeleccionaColor(sh_programID, col_object, sw_mat);
	ModelMatrix = glm::translate(MatriuTG, vec3(-10.0, 0.0, 3.4));
	ModelMatrix = glm::rotate(ModelMatrix, radians(270.f), vec3(1.0, 0.0, 0.0f));
	...
	//Propulsor 2 detall davanter (torus)
	col_object.r = 0.49;	col_object.g = 0.49;		col_object.b = 0.49;	 col_object.a = 1.0;
	SeleccionaColor(sh_programID, col_object, sw_mat);
	ModelMatrix = glm::translate(MatriuTG, vec3(-10.0, 6.0, 3.4));
	ModelMatrix = glm::rotate(ModelMatrix, radians(270.f), vec3(1.0f, 0.0f, 0.0f));
	...
	col_object.r = 0.49;	col_object.g = 0.49;		col_object.b = 0.49;	 col_object.a = 1.0;
	SeleccionaColor(sh_programID, col_object, sw_mat);
	ModelMatrix = glm::translate(MatriuTG, vec3(9.0, -16.0, 5.0));
	ModelMatrix = glm::scale(ModelMatrix, vec3(8.0f, 3.0f, 0.6f));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
	NormalMatrix = transpose(inverse(MatriuVista * ModelMatrix));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "normalMatrix"), 1, GL_FALSE, &NormalMatrix[0][0]);
	glutSolidCube(1.0);

	//Punta aleró horitzontal 1
	col_object.r = 0.49;	col_object.g = 0.49;		col_object.b = 0.49;	 col_object.a = 1.0;
	SeleccionaColor(sh_programID, col_object, sw_mat);
	ModelMatrix = glm::translate(MatriuTG, vec3(13.0, -16.0, 5.0));
	ModelMatrix = glm::scale(ModelMatrix, vec3(1.0f, 1.5f, 0.35f));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
	NormalMatrix = transpose(inverse(MatriuVista * ModelMatrix));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "normalMatrix"), 1, GL_FALSE, &NormalMatrix[0][0]);
	glutSolidSphere(1.0, 20, 20);
	...
	col_object.r = 0.49;	col_object.g = 0.49;		col_object.b = 0.49;	 col_object.a = 1.0;
	SeleccionaColor(sh_programID, col_object, sw_mat);
	ModelMatrix = glm::translate(MatriuTG, vec3(0.0, -16.0, 12.0));
	ModelMatrix = glm::scale(ModelMatrix, vec3(0.6f, 3.0f, 4.0f));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
	NormalMatrix = transpose(inverse(MatriuVista * ModelMatrix));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "normalMatrix"), 1, GL_FALSE, &NormalMatrix[0][0]);
	glutSolidCube(1.0);

	//Punta aleró vertical
	col_object.r = 0.49;	col_object.g = 0.49;		col_object.b = 0.49;	 col_object.a = 1.0;
	SeleccionaColor(sh_programID, col_object, sw_mat);
	ModelMatrix = glm::translate(MatriuTG, vec3(0.0, -16.0, 14.0));
	ModelMatrix = glm::scale(ModelMatrix, vec3(0.35f, 1.5f, 1.0f));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
	NormalMatrix = transpose(inverse(MatriuVista * ModelMatrix));
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "normalMatrix"), 1, GL_FALSE, &NormalMatrix[0][0]);
	glutSolidSphere(1.0, 20, 20);
...
// Matrius Transformació
	...
	CColor col_object;
	int nvertD = gluDisk_VAO(0.0f, 5.0f, 6, 1);
...
// Definició propietats de reflexió (emissió, ambient, difusa, especular) del material.
	  col_object.r = 1.0;	col_object.g = 1.0;		col_object.b = 1.0;	 col_object.a = 1.0;
	  SeleccionaColor(shaderId, col_object, sw_mat);
